#!/usr/bin/env python3
"""Build flat offline pack + site wiring for 1º ESO Inglés / Lengua Extranjera (40/40).
...
PACK_DIR = REPO / "downloads/_build-1eso-ingles-flat"
ZIP_PATH = REPO / "downloads/1eso-ingles-offline.zip"
BRAND_ICONS = COURSE_DIR / "icons"
CALC = REPO / "modulos/calculadora.html"

TOTAL = 40
AVAILABLE = 40
COURSE_NAME = "1º ESO Inglés"
COURSE_SHORT = "1º ESO Inglés"
ZIP_FOLDER = "1eso-ingles-offline"
HUB_BASENAME = "1eso-ingles.html"
ZIP_BASENAME = "1eso-ingles-offline.zip"
ARTICLE_NUM = "08i"


def discover_course_index() -> tuple[dict[int, str], list[str]]:
    """Discover named shells and titles only from the English (Inglés) course."""
    files = {}
    ...
    if len(files) != TOTAL or set(files) != set(range(1, TOTAL + 1)):
        raise SystemExit(f"ingles: expected {TOTAL} named shells, found {len(files)}")
...
def pack_ud_blurb(*, short: bool = False) -> str:
    if short:
        return "currículo CyL Decreto 39/2022 LOMLOE · lengua extranjera inglés"
    return (
        "Lengua Extranjera (Inglés) 1º ESO: currículo de Castilla y León "
        "(Decreto 39/2022, LOMLOE); comunicación, plurilingüismo e interculturalidad; "
        "no es Francés ni otra lengua extranjera"
    )
...
def refresh_hub_footnote() -> None:
    ...
    new_status_inner = (
        ...
        "(Lengua Extranjera · Inglés · Decreto 39/2022 CyL LOMLOE, 100&nbsp;%).\n"
        "      No es Francés ni otra lengua extranjera. Curso cerrado · Revisor Dios <strong>CONFIRMA</strong> (0 críticos).\n"
    ...
    new_pie = (
        '<p class="hub-pie-nota">Material educativo de Lengua Extranjera (Inglés) 1º ESO. '
        "Basado en el currículo de Castilla y León (Decreto 39/2022, LOMLOE). "
        "No es Francés ni sustituye al criterio del centro o del profesorado. "
    ...
    if re.search(r'<p class="hub-pie-nota">', text2):
        ...
        # Inglés hub uses a plain footer; insert hub-pie-nota before it and add CSS once.
...
def write_downloads_mirror() -> None:
    ...
    doc = doc.replace('href="icons/', 'href="../profesor/1eso-ingles/icons/')
    doc = doc.replace('href="manifest.webmanifest"', 'href="../profesor/1eso-ingles/manifest.webmanifest"')
    doc = doc.replace('href="../_plantilla-leccion/', 'href="../profesor/_plantilla-leccion/')
    doc = doc.replace('href="lecciones/', 'href="../profesor/1eso-ingles/lecciones/')
    doc = doc.replace('href="../../descargas.html"', 'href="../descargas.html"')
    doc = doc.replace('href="../../index.html"', 'href="../index.html"')
    (REPO / "downloads/1eso-ingles.html").write_text(doc, encoding="utf-8")
    print("Wrote downloads/1eso-ingles.html mirror")
...
def update_descargas() -> None:
    ...
    lead_new = (
        ...
        "(pack completo L01–L30/30) e Inglés / Lengua Extranjera "
    ...
    article = f"""        <article class="item">
          ...
          <div>
            ...
            Lengua Extranjera (Inglés); <strong>no es Francés</strong>. Modo maestro en cada lección. Sin nube ni servidor. Distinto del pack Profesor.</p>
            <div class="actions">
              <a class="btn-download" href="/downloads/{ZIP_BASENAME}" download="{ZIP_BASENAME}">Descargar ZIP</a>
              <a class="textlink" href="/profesor/1eso-ingles/lecciones/{lesson_filename(1)}">Abrir lección 01</a>
              <a class="textlink" href="/profesor/1eso-ingles/{HUB_BASENAME}">Índice del curso</a>
            </div>
          </div>
        </article>
"""
    if ZIP_BASENAME in text:
        ...
        if n:
            ...
            raise SystemExit("descargas.html: existing ingles article not replaced")
    else:
        anchor = 'href="/downloads/1eso-religion-offline.zip"'
        idx = text.find(anchor)
        if idx < 0:
            raise SystemExit("descargas.html: religion download anchor not found")
        end = text.find("</article>", idx) + len("</article>")
        text = text[:end] + "\n" + article + text[end:]
    DESCARGAS.write_text(text, encoding="utf-8")
    print("descargas.html: ingles article", ARTICLE_NUM)
...
def update_descargas_profesor() -> None:
    ...
    article = f"""      <article class="item">
        ...
        <div>
          <h2>{COURSE_NAME}</h2>
          <p class="kicker">Oficial CyL · pack completo {AVAILABLE}/{TOTAL} · lengua extranjera</p>
          <div class="actions">
            <a class="btn-download" href="/downloads/{ZIP_BASENAME}" download="{ZIP_BASENAME}">Descargar ZIP</a>
            <a class="textlink" href="/profesor/1eso-ingles/{HUB_BASENAME}">Índice del curso</a>
          </div>
...
"""
    if ZIP_BASENAME in text:
        ...
        idx = text.find(anchor)
        if idx < 0:
            raise SystemExit("descargas-profesor: religion download anchor not found")
        end = text.find("</article>", idx) + len("</article>")
        text = text[:end] + "\n" + article + text[end:]
    DESCARGAS_PROF.write_text(text, encoding="utf-8")
    print("descargas-profesor: ingles article", ARTICLE_NUM)


def update_index() -> None:
    text = INDEX.read_text(encoding="utf-8")
    block = f"""              <li>
                <span class="lv-pack-name">{COURSE_NAME} <em>(pack completo {AVAILABLE}/{TOTAL} · lengua extranjera)</em></span>
                <span class="lv-cta-row lv-cta-inline">
                  <a class="lv-square lv-square-secondary" href="/profesor/1eso-ingles/{HUB_BASENAME}">Ver</a>
                  <a class="lv-square lv-square-secondary" href="/downloads/{ZIP_BASENAME}" download="{ZIP_BASENAME}">Descargar</a>
                </span>
              </li>
"""
    ingles_row = r'\s*<li>\s*<span class="lv-pack-name">1º ESO Inglés.*?</li>\n?'
    text = re.sub(ingles_row, "", text, count=1, flags=re.S)
    rel = re.search(
        r'<li>\s*<span class="lv-pack-name">1º ESO Religión Católica.*?</li>',
        text,
        re.S,
    )
    if not rel:
        raise SystemExit("index.html: religion row not found")
    text = text[: rel.end()] + "\n" + block + text[rel.end() :]
    text = re.sub(
        r'(</span>) +(?=</li>\s*<li>\s*<span class="lv-pack-name">1º ESO Inglés)',
        r"\1\n              ",
        text,
        count=1,
    )
    INDEX.write_text(text, encoding="utf-8")
    print("index.html: ingles pillar row")
...
def update_inventory_light() -> None:
    ...
    bullet = (
        f"- **1º ESO Inglés / Lengua Extranjera**: pack completo L01–L{AVAILABLE:02d}/{TOTAL} "
    ...
    if marker in text:
        ...
        if m:
            body = m.group(2)
            if "1º ESO Inglés" in body:
                body2, n = re.subn(r"- \*\*1º ESO Inglés[^*]*\*\*:.*?\n", bullet, body, count=1)
    ...
    print("inventory: ingles publication bullet")
...
**Qué es:** pack de **Lengua Extranjera (Inglés)** de 1º ESO (currículo de Castilla y León, Decreto 39/2022, LOMLOE). **No es Francés.**
...
**Identidad:** material educativo de **Lengua Extranjera (Inglés)**. Se basa en Decreto 39/2022 (LOMLOE). **No es Francés.** No sustituye al criterio del centro ni del profesorado.
...
<div class="leccion-wrap">
  ...
  <div class="aviso-salud">
    <strong>Identidad:</strong> Lengua Extranjera (Inglés) · <strong>no</strong> es Francés.
...
"""
    ...
    # Ship the named interactive shells used by the long lesson aliases.
