#!/usr/bin/env python3
"""Build flat offline pack + site wiring for 1º ESO Religión Católica (30/30).
...
PACK_DIR = REPO / "downloads/_build-1eso-religion-flat"
ZIP_PATH = REPO / "downloads/1eso-religion-offline.zip"
BRAND_ICONS = COURSE_DIR / "icons"
CALC = REPO / "modulos/calculadora.html"

TOTAL = 30
AVAILABLE = 30
COURSE_NAME = "1º ESO Religión Católica"
COURSE_SHORT = "1º ESO Religión Católica"
ZIP_FOLDER = "1eso-religion-offline"
HUB_BASENAME = "1eso-religion.html"
ZIP_BASENAME = "1eso-religion-offline.zip"
ARTICLE_NUM = "08h"

def discover_course_index() -> tuple[dict[int, str], list[str]]:
    """Discover named shells and titles only from the Catholic religion course."""
    files = {}
    for path in sorted(LEC.glob("leccion-[0-9][0-9]-*.html")):
        m = re.match(r"leccion-(\d{2})-", path.name)
        if m:
            files[int(m.group(1))] = path.name
    if len(files) != TOTAL or set(files) != set(range(1, TOTAL + 1)):
        raise SystemExit(f"religion: expected {TOTAL} named shells, found {len(files)}")
    hub = HUB.read_text(encoding="utf-8")
    titles = {}
    for m in re.finditer(r'href="lecciones/(leccion-(\d{2})-[^"]+\.html)"[^>]*>.*?<span class="hub-titulo">(.*?)</span>', hub, re.S):
...
def pack_ud_blurb(*, short: bool = False) -> str:
    if short:
        return "marco confesional BOE-A-2022-10452 · Decreto 39/2022"
    return (
        "Religión Católica 1º ESO: saberes básicos A–C del currículo confesional "
        "(BOE-A-2022-10452, Anexo ESO) y marco de Castilla y León (Decreto 39/2022); "
        "no es Alternativa laica"
    )
...
def refresh_hub_footnote() -> None:
    ...
    new_status_inner = (
        ...
        "(Religión Católica · marco confesional BOE-A-2022-10452 / Decreto 39/2022, 100&nbsp;%).\n"
        "      No es Alternativa laica. Curso cerrado · Revisor Dios <strong>CONFIRMA</strong> (0 críticos).\n"
        "      <strong>ZIP offline</strong> (sin instalar: descomprime y abre "
        '<code>ABRE-AQUI.html</code>) en <a href="../../descargas.html">Descargas</a>.'
    )
    text2, n = re.subn(r'(<p class="hub-status">).*?(</p>)', r"\1\n" + new_status_inner + r"\n    \2", text, count=1, flags=re.S)
    if not n:
        raise SystemExit("hub: hub-status not found")
    new_pie = (
        '<p class="hub-pie-nota">Material educativo confesional de Religión Católica. '        'Basado en BOE-A-2022-10452 y Decreto 39/2022. No es Alternativa laica ni sustituye al criterio del centro o del profesorado. '        "Curso cerrado · Revisor Dios CONFIRMA (0 críticos).</p>"
    )
    text2, n = re.subn(r'<p class="hub-pie-nota">.*?</p>', new_pie, text2, count=1, flags=re.S)
    if not n:
        raise SystemExit("hub: hub-pie-nota not found")
    n_after = len(re.findall(r'href="lecciones/leccion-\d{2}-[^"]+\.html"', text2))
...
def write_downloads_mirror() -> None:
    ...
    doc = doc.replace('href="icons/', 'href="../profesor/1eso-religion/icons/')
    doc = doc.replace('href="manifest.webmanifest"', 'href="../profesor/1eso-religion/manifest.webmanifest"')
    doc = doc.replace('href="../_plantilla-leccion/', 'href="../profesor/_plantilla-leccion/')
    doc = doc.replace('href="lecciones/', 'href="../profesor/1eso-religion/lecciones/')
    doc = doc.replace('href="../../descargas.html"', 'href="../descargas.html"')
    doc = doc.replace('href="../../index.html"', 'href="../index.html"')
    (REPO / "downloads/1eso-religion.html").write_text(doc, encoding="utf-8")
    print("Wrote downloads/1eso-religion.html mirror")
...
def update_descargas() -> None:
    ...
    lead_new = (
        ...
        "Alternativa laica a la Religión (pack completo L01–L28/28) y Religión Católica "
        f"(pack completo L01–L{AVAILABLE:02d}/{TOTAL}). Religión Católica conforme al marco confesional "
    ...
    m = re.search(r'<p class="lead-mini">Currículo oficial Castilla y León \(LOMLOE\).*?</p>', text, re.S)
    if not m:
    ...
    article = f"""        <article class="item">
          ...
          <div>
            <h2>{COURSE_NAME}</h2>
            <p class="kicker">Oficial CyL · marco confesional BOE-A-2022-10452 · pack completo ({TOTAL} lecciones)</p>
            ...
            Religión Católica confesional; <strong>no es Alternativa laica</strong>. Modo maestro en cada lección. Sin nube ni servidor. Distinto del pack Profesor.</p>
            <div class="actions">
              <a class="btn-download" href="/downloads/{ZIP_BASENAME}" download="{ZIP_BASENAME}">Descargar ZIP</a>
              <a class="textlink" href="/profesor/1eso-religion/lecciones/{lesson_filename(1)}">Abrir lección 01</a>
              <a class="textlink" href="/profesor/1eso-religion/{HUB_BASENAME}">Índice del curso</a>
            </div>
          </div>
        </article>
"""
    if ZIP_BASENAME in text:
        text2, n = re.subn(rf'\s*<article class="item">\s*<div class="num">{ARTICLE_NUM}</div>.*?</article>\n', "\n" + article, text, count=1, flags=re.S)
        if n:
            text = text2
        else:
            raise SystemExit("descargas.html: existing religion article not replaced")
    else:
        anchor = 'href="/downloads/1eso-alternativa-religion-offline.zip"'
        idx = text.find(anchor)
        if idx < 0:
            raise SystemExit("descargas.html: alternative download anchor not found")
        end = text.find("</article>", idx) + len("</article>")
        text = text[:end] + "\n" + article + text[end:]
    DESCARGAS.write_text(text, encoding="utf-8")
    print("descargas.html: religion article", ARTICLE_NUM)

def update_descargas_profesor() -> None:
    text = DESCARGAS_PROF.read_text(encoding="utf-8")
    text = text.replace(
        "1º ESO CyL (Mate, ByG, GeoHistoria, Lengua, Plástica, Educación Física, Alternativa laica)",
        "1º ESO CyL (Mate, ByG, GeoHistoria, Lengua, Plástica, Educación Física, Alternativa laica, Religión Católica)",
    )
    article = f"""      <article class="item">
        <div class="num">{ARTICLE_NUM}</div>
        <div>
          <h2>{COURSE_NAME}</h2>
          <p class="kicker">Oficial CyL · pack completo {AVAILABLE}/{TOTAL} · confesional</p>
          <div class="actions">
            <a class="btn-download" href="/downloads/{ZIP_BASENAME}" download="{ZIP_BASENAME}">Descargar ZIP</a>
            <a class="textlink" href="/profesor/1eso-religion/{HUB_BASENAME}">Índice del curso</a>
          </div>
...
"""
    if ZIP_BASENAME in text:
        ...
        idx = text.find(anchor)
        if idx < 0:
            raise SystemExit("descargas-profesor: alternative download anchor not found")
        end = text.find("</article>", idx) + len("</article>")
        text = text[:end] + "\n" + article + text[end:]
    DESCARGAS_PROF.write_text(text, encoding="utf-8")
    print("descargas-profesor: religion article", ARTICLE_NUM)

def update_index() -> None:
    text = INDEX.read_text(encoding="utf-8")
    block = f"""              <li>
                <span class="lv-pack-name">{COURSE_NAME} <em>(pack completo {AVAILABLE}/{TOTAL} · confesional)</em></span>
                <span class="lv-cta-row lv-cta-inline">
                  <a class="lv-square lv-square-secondary" href="/profesor/1eso-religion/{HUB_BASENAME}">Ver</a>
                  <a class="lv-square lv-square-secondary" href="/downloads/{ZIP_BASENAME}" download="{ZIP_BASENAME}">Descargar</a>
                </span>
              </li>
"""
    religion_row = r'\s*<li>\s*<span class="lv-pack-name">1º ESO Religión Católica.*?</li>\n?'
    text = re.sub(religion_row, "", text, count=1, flags=re.S)
    alt = re.search(r'<li>\s*<span class="lv-pack-name">1º ESO Alternativa a la Religión.*?</li>', text, re.S)
    if not alt:
        raise SystemExit("index.html: alternative row not found")
    text = text[:alt.end()] + "\n" + block + text[alt.end():]
    text = re.sub(r'(</span>) +(?=</li>\s*<li>\s*<span class="lv-pack-name">1º ESO Religión Católica)', r'\1\n              ', text, count=1)
    INDEX.write_text(text, encoding="utf-8")
    print("index.html: religion pillar row")
...
def update_inventory_light() -> None:
    ...
    bullet = (
        f"- **1º ESO Religión Católica**: pack completo L01–L{AVAILABLE:02d}/{TOTAL} "
        "(hub + ZIP offline, marco confesional BOE-A-2022-10452). Revisor Dios CONFIRMA (0 críticos). "
        "No es Alternativa laica.\n"
    )
    marker = "### Publicación completa (Les vencimos)"
    if marker in text:
        m = re.search(r"(### Publicación completa \(Les vencimos\)\n)(.*?)(?=\n### |\Z)", text, re.S)
        if m:
            body = m.group(2)
            if "1º ESO Religión Católica" in body:
                body2, n = re.subn(r"- \*\*1º ESO Religión Católica\*\*:.*?\n", bullet, body, count=1)
    ...
    print("inventory: religion publication bullet")
...
"""
    ...
    # Ship the 30 named interactive shells used by the long lesson aliases.
